...
const rooms = {
  ...
  archive: {
    ...
    demo: ["осмотреть стол", "прослушать запись", "взять архивный модуль", "что осталось", "идти в серверную"],
    tasks: [
  ...
  server: {
    ...
    demo: ["осмотреть стойку", "вставить модуль", "расшифровать отчет", "куда идти", "идти к генератору"],
    tasks: [
  ...
  lab: {
    ...
    demo: ["осмотреть лабораторию", "взять стабилизатор", "восстановить охлаждение", "что осталось", "идти в обсерваторию"],
    tasks: [
  ...
  observatory: {
    ...
    demo: ["осмотреть консоль", "что нужно для запуска", "взять кристалл", "синхронизировать антенну", "запусти портал", "отправить сигнал", "эвакуироваться", "аварийный запуск"],
    tasks: [
...
const words = {
  ...
  reset: ["начать заново", "сброс", "перезапуск", "новая игра"],
};
...
function handleCommandPreview(command) {
  ...
  if (hasAny(command, words.reset) || hasAny(command, words.repeat) || hasAny(command, words.help) || hasAny(command, words.where) || hasAny(command, words.remaining) || hasAny(command, words.next) || hasAny(command, words.inventory) || hasAny(command, words.stats)) return true;
...
function runCommand(rawCommand, source = "voice") {
  ...
  const command = expandCommand(normalizedRaw);
  if (!command) return;
  ...
  dom.lastCommand.textContent = `Распознано: ${rawCommand}`;
  setVoiceStatus("Команда принята", "ready");

  if (hasAny(command, words.reset)) return resetGame();
  if (hasAny(command, words.repeat)) return finishWith(() => repeatLastMessage());
  if (hasAny(command, words.help)) return finishWith(() => describeHelp());
  if (hasAny(command, words.where)) return finishWith(() => describeLocation());
  if (hasAny(command, words.remaining)) return finishWith(() => describeRemaining());
  if (hasAny(command, words.next) && !hasAny(command, words.go)) return finishWith(() => describeNextStep());
  if (hasAny(command, words.inventory)) return finishWith(() => describeInventory());
  if (hasAny(command, words.stats)) return finishWith(() => addLog(getStatsText(), true, "info"));
...
function handleNavigation(command) {
  ...
  if (isFinaleRoom(state.room)) {
    addLog("Маршрут уже завершен. Можно начать заново.");
    return true;
  }
...
const roomHandlers = {
  atrium(command) {
    ...
    if ((hasAny(command, words.use) || hasAny(command, words.open) || hasAny(command, words.go)) && hasAny(command, ["пропуск", "карта", "карточка", "бейдж", "дверь", "архив", "проход", "доступ", "пульт", "считыватель", "дальше", "вперед"])) {
      ...
      state.room = "archive";
      addLog("Пульт принял пропуск. Дверь открылась, вы проходите в архив.", true, "success");
      return true;
    }
...
function describeNextStep() {
  if (isFinaleRoom(state.room)) {
    addLog("Квест завершен. Можно начать заново.");
    return;
  }
...
function describeHelp() {
  if (isFinaleRoom(state.room)) {
    addLog("Маршрут завершен. Доступны итоги, статистика или новая игра.", true, "info");
    return;
  }
  const missing = getMissingRequired(state.room);
  if (missing.length) {
...
function renderNavigation() {
  if (isFinaleRoom(state.room)) {
    dom.nextStep.textContent = "Квест завершен. Можно запросить статистику или начать заново.";
    return;
  }
  const missing = getMissingRequired(state.room);
...
function renderLog() {
  ...
  state.log.forEach((entry) => {
    ...
    li.classList.toggle("is-success", item.type === "success");
    li.classList.toggle("is-warning", item.type === "warning");
